...
nonsynack_df = shuffled_df.loc[(shuffled_df['subcategory'] != "synflooding") & (shuffled_df['subcategory'] != "ackflooding")]
#loc函数：通过行索引 "Index" 中的具体值来取行数据（如取"Index"为"A"的行）

syn_df = shuffled_df.loc[shuffled_df['subcategory'] == "synflooding"].sample(n=10307,random_state=42)
#syn_df = shuffled_df.loc[shuffled_df['subcategory'] == "synflooding"].sample(n=73122,random_state=42)
ack_df = shuffled_df.loc[shuffled_df['subcategory'] == "ackflooding"].sample(n=10307,random_state=42)
