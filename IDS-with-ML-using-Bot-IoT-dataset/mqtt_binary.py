...
full_data = pd.read_csv(full_path,usecols=['flow_duration','flow_iat_mean','flow_pkts_s','flow_iat_max','bwd_pkt_len_max','totlen_fwd_pkts','totlen_fwd_pkts','tot_bwd_pkts','fwd_pkt_len_max','fwd_pkt_len_max','fwd_pkt_len_min','fwd_pkt_len_mean','bwd_pkt_len_min','bwd_pkt_len_mean','fwd_iat_tot','subcategory'])
...
#full_data.drop(['src_ip', 'dst_ip', 'dst_ip', 'dst_ip', 'dst_ip',"label","category"],axis=1,inplace=True)
#full_data = pd.concat([train_data,test_data],axis=0)
#print(test_data)
xxx = full_data.loc[full_data.subcategory=='subcategory']
...
print("full_data.subcategory.value_counts():")
#synflooding       61248
#ackflooding       32332
#portos            10307
#hostport           7390
...
fig = px.bar(d, x=d.index, y=d.values,title = 'Class distribution between attack subcategories on full data in IoTID20',labels = {'index':'Attack','y':'Volume'},color=d.values,text_auto='.2s')
...
# %%
d = full_data.subcategory.value_counts()
fig = px.bar(d, x=d.index, y=d.values,title = 'Class Label Distribution in IoTID20',labels = {'index':'Attack','y':'Volume'},color=d.values,text_auto='.2s')
fig.update_layout(title_x=0.5,width=1000, height=400)
fig.show()
...
nonspartabruteforce_df = shuffled_df.loc[(shuffled_df['subcategory'] != "sparta") & (shuffled_df['subcategory'] != "mqtt_bruteforce")]
#loc函数：通过行索引 "Index" 中的具体值来取行数据（如取"Index"为"A"的行）

sparta_df = shuffled_df.loc[shuffled_df['subcategory'] == "sparta"].sample(n=88577,random_state=42)
#syn_df = shuffled_df.loc[shuffled_df['subcategory'] == "synflooding"].sample(n=73122,random_state=42)
bruteforce_df = shuffled_df.loc[shuffled_df['subcategory'] == "mqtt_bruteforce"].sample(n=88577,random_state=42)

normalized_full_df = pd.concat([nonspartabruteforce_df, sparta_df, bruteforce_df])
#normalized_full_df = shuffled_df
#pandas.concat()通常用来连接DataFrame对象，默认纵向，axis=1 横向连接

#TRAIN after undersampling
d = normalized_full_df.subcategory.value_counts()
fig = px.bar(d, x=d.index, y=d.values,title = 'Class Label Distribution in IoTID20 (undersampled)',labels = {'index':'Attack','y':'Volume'},color=d.values,text_auto='.2s')
fig.update_layout(title_x=0.5,width=1000, height=400)
fig.show()
...
print('X = normalized_full_df.drop(["subcategory"], axis = 1),finish')
